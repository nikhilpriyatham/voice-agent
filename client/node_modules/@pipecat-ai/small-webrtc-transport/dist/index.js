...
class $8381b73e1f76ead9$export$62043589d053a879 extends (0, $7ytRB$pipecataiclientjs.Transport) {
    constructor(opts = {}){
        ...
        this.mediaManager = new (0, $1c088932741d88e6$export$c95c65abc5f47125)(false, false, async (event)=>{
            if (!this.pc) return;
            if (event.type == "audio") {
    ...
    _validateConnectionParams(connectParams) {
        ...
        if (typeof connectParams !== "object") throw new (0, $7ytRB$pipecataiclientjs.RTVIError)("Invalid connection parameters");
        for (const [key, val] of Object.entries(connectParams)){
            if (key !== "connectionUrl") throw new (0, $7ytRB$pipecataiclientjs.RTVIError)(`Unrecognized connection parameter: ${key}. Only 'connectionUrl' is allowed.`);
            else if (typeof val !== "string") throw new (0, $7ytRB$pipecataiclientjs.RTVIError)(`Invalid type for connectionUrl: expected string, got ${typeof val}`);
        }
    ...
    async _connect(connectParams) {
        ...
        if (!this._connectionUrl) {
            (0, $7ytRB$pipecataiclientjs.logger).error("No url provided for connection");
            this.state = "error";
    ...
    createPeerConnection() {
        ...
        pc.addEventListener("icegatheringstatechange", ()=>{
            (0, $7ytRB$pipecataiclientjs.logger).debug(`iceGatheringState: ${this.pc.iceGatheringState}`);
        });
        (0, $7ytRB$pipecataiclientjs.logger).debug(`iceGatheringState: ${pc.iceGatheringState}`);
        ...
        pc.addEventListener("track", (evt)=>{
            (0, $7ytRB$pipecataiclientjs.logger).debug(`Received new track ${evt.track.kind}`);
    ...
    async negotiate(recreatePeerConnection = false) {
        ...
        try {
            ...
            (0, $7ytRB$pipecataiclientjs.logger).debug(`Remote candidate supports trickle ice: ${this.pc.canTrickleIceCandidates}`);
        } catch (e) {
    ...
    enableScreenShare(enable) {
        (0, $7ytRB$pipecataiclientjs.logger).error("startScreenShare not implemented for SmallWebRTCTransport");
        throw new Error("Not implemented");
    }
    get isSharingScreen() {
        (0, $7ytRB$pipecataiclientjs.logger).error("isSharingScreen not implemented for SmallWebRTCTransport");
        return false;
    }

...
async def run_bot(room_url: str, token: str, patient_name: str, device_ordered: str):
    ...
    async def on_first_participant_joined(transport, participant):
        ...
        # 2. Retrieve past conversation history from Mem0 for personalization
        past_context = None
        if mem0_client:
            try:
        ...
        # 3. Start transcription capture (audio recording via Daily)
        await transport.capture_participant_transcription(participant["id"])

        # 4. Initialize flow manager and begin conversation
